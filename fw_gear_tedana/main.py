...
def run(gear_options: dict, app_options: dict) -> int:
    """Run tedana using fmriprep or manual inputs.
    ...
    if app_options["inputtype"] == "fmriprep":
        ...
        for task in tasks:
            ...
            index = [idx for idx, s in enumerate(echo_files[0].split("_")) if 'echo' in s][0]
            ...
            if app_options["output-layout"] == "bids":
                output_analysis_id_dir = op.join(gear_options["work-dir"],gear_options["destination-id"],"fmriprep","derivatives","tedana", "sub-" + app_options["sid"], "ses-" + app_options["sesid"],"func")
                os.makedirs(output_analysis_id_dir, exist_ok=True)
                arg_options["out-dir"] = output_analysis_id_dir
            else:
                output_analysis_id_dir = op.join(gear_options["work-dir"], gear_options["destination-id"], "fmriprep", "sub-" + app_options["sid"], "ses-" + app_options["sesid"],"func")
                os.makedirs(output_analysis_id_dir, exist_ok=True)
                arg_options["out-dir"] = output_analysis_id_dir
...
def fmriprep_get_tasks(path):
    ...
    results = []
    files = searchfiles(os.path.join(path,"func","*bold.nii.gz"))
    for f in files:
        name_split = f.split("_")
        res = [i for i in name_split if "task" in i][0]
        results.append(res)
